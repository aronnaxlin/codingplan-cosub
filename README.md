...
一个面向小团队内部使用的 Coding Plan 薄代理和管理面板。界面和部分功能来源于 [sub2api](https://github.com/Wei-Shaw/sub2api)。
...
CODING_PLAN_QUOTA_USER_AGENT=CodingPlanProxy/0.1 quota-check
```
...
```bash
cp .env.example .env
# 编辑 .env，设置 ADMIN_TOKEN 和 CODING_PLAN_API_KEY
```
...
管理面板可以手动刷新官方额度。这个功能**默认关闭**，因为它会额外向上游发起一次带 `User-Agent` 的请求；确认接受这个风险后，在设置页打开“启用官方额度检查”再使用。
